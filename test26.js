...
const App = () => {
  ...
  const loadSurveys = async (retryCount = 0, maxRetries = 5, delay = 2000) => {
    if (!currentUser) {
      console.error('loadSurveys: currentUser is undefined');
      addNotification('Cannot load surveys: User information not available.', 'error');
    ...
    console.log('Loading surveys for userId:', userId, 'isSiteAdmin:', isSiteAdmin, 'attempt:', retryCount + 1); // Debug
    ...
    try {
      ...
      if (!response.d.results) {
        ...
        addNotification('Survey data unavailable. API returned undefined results.', 'error');
        setIsLoadingSurveys(false);
        return;
      }
      ...
      const updatedSurveys = await Promise.all(surveys.map(async s => {
        try {
          ...
          console.log(`Response API result for survey ${s.Id}:`, res.d.results); // Debug
          const count = res.d.__count !== undefined ? parseInt(res.d.__count, 10) : res.d.results.length || 0;
          if (!res.d.results || res.d.results.length === 0) {
            console.warn(`No responses found for survey ${s.Id} or query returned empty results.`);
          }
  ...
  return (
    <div className="flex flex-col h-screen">
      ...
      <TopNav username={currentUser?.get_title()} toggleSideNav={() => setIsSideNavOpen(!isSideNavOpen)} />
...
const TopNav = ({ username, toggleSideNav }) => (
...
const SideNav = ({ filters, onFilterChange, isOpen, toggle }) => {
  ...
  return (
    <>
      ...
      <div 
        className={`fixed top-0 left-0 h-full w-4/5 max-w-sm bg-gray-100 p-4 z-50 transition-transform transform md:relative md:w-1/4 md:transform-none ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}
    ...
    <div className="border p-4 rounded shadow bg-white hover:shadow-lg transition flex flex-col">
      <div className="flex-1">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {survey.Owners?.results?.length > 0 ? (
            survey.Owners.results.map(owner => (
              <span
                key={owner.Id}
          ...
          <span
            ...
            aria-label={`Response count: ${survey.responseCount ?? 'Error'}`}
          >
            Responses: {survey.responseCount ?? 'Error'}
          </span>
        </div>
